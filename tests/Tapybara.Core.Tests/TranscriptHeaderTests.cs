...
public class TranscriptHeaderTests
{
    ...
    public void FirstUtteranceGetsAHeaderAndDoesNotOverflow() =>
        Assert.True(CallTranscriptRenderer.NeedsHeader(At(0), "Кирилл", headerAt: null, headerSpeaker: null));

    [Fact]
    public void FirstUtteranceOfALongCallDoesNotOverflowEither() =>
        Assert.True(CallTranscriptRenderer.NeedsHeader(At(60), "Кирилл", headerAt: null, headerSpeaker: null));

    [Fact]
    public void ChangeOfSpeakerGetsAHeader() =>
        Assert.True(CallTranscriptRenderer.NeedsHeader(At(1), "Марина", At(0.9), "Кирилл"));

    /// <summary>Подряд идущие реплики одного человека не штампуются именем.</summary>
    [Fact]
    public void SameSpeakerShortlyAfterDoesNot() =>
        Assert.False(CallTranscriptRenderer.NeedsHeader(At(1), "Кирилл", At(0.9), "Кирилл"));
    ...
    public void SameSpeakerAfterALongStretchGetsAHeaderAgain() =>
        Assert.True(CallTranscriptRenderer.NeedsHeader(At(4), "Кирилл", At(1), "Кирилл"));

    [Fact]
    public void TheIntervalIsTwoMinutes()
    {
        Assert.False(CallTranscriptRenderer.NeedsHeader(At(2.9), "Кирилл", At(1), "Кирилл"));
        Assert.True(CallTranscriptRenderer.NeedsHeader(At(3.0), "Кирилл", At(1), "Кирилл"));
    }
}
