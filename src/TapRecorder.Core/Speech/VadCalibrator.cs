...
/// </para>
/// <para>
/// Рекомендуется самый СТРОГИЙ порог из тех, что ещё не теряют речь. Строже —
/// меньше ложных срабатываний на шуме и щелчках; но потерянное слово не
...
public static class VadCalibrator
{
    ...
    public static float? Recommend(IReadOnlyList<VadProbe> probes)
    {
        ...
        return acceptable.Length == 0 ? null : acceptable.Max(p => p.Threshold);
    }
}
